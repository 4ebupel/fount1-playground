...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Debug logs
    console.log('Request URL:', `${process.env.XANO_API_GROUP_BASE_URL}/companies/update/banner`);
    console.log('Form Data Headers:', xanoFormData.getHeaders());

    const response = await apiClient.patch(
      `${process.env.XANO_API_GROUP_BASE_URL}/companies/update/banner`,
      xanoFormData,
      {
        headers: {
    ...
    return basicErrorHandler(error, "Error uploading company banner");
  }
}
